##### IIII. scDNA: SubClustering()
##### scDNAclustering_05_SubcloneClustering.R

# 5.1: collect_cluster_bp() collect all break points in the cluster
#'==============================================================================
...
# 5.2: output_bp_covers() calcuate each bp with cover bps
#'==============================================================================
...
#' @param overlap_times A numeric value used as a filtering criterion for significant 
#'  breakpoints in event region calculation. The default value is set as half the 
...
# 5.3: bp_events() use the distribution of breakpoints to define the events
#'==============================================================================
...
#'  copy number variation calling result where the genomic region is split into bins.
#'
#' @importFrom magrittr %>%
#' @importFrom rlang .data
#'
#' @return A data frame listing CNV events for each chromosome, identifying regions 
#'  where copy number variations frequently occur.
#'
#' @keywords internal
#'
bp_events <- function(input, Template, binsize)
{
  fucStep <- " 5.3_cnvTree_v030"
  DebugMsg(fucStep, "start")
...
# 5.3.1: event_region.bin() change event_region to bins level(sites)
#'==============================================================================
    ...
    if(event_R$region[1] == 0){  # some chrmosome with no breakpoint
      ...
      event_R <- event_R %>%
        ...
        dplyr::select(.data$chr, .data$start, .data$end, .data$region, 
                      .data$region_size, .data$region_ratio, .data$event_binstart, 
                      .data$event_binend)
    }
...
# 5.6: Subclone_clustering() check cell to cell whether with >?% of different 
# chromosome than clustering
#'==============================================================================
...
# 5.7: Subclone_CNregion() output each region copy number in each subclone, 
# as the basement of CNV template
#'==============================================================================
...
#' @param min_cell An integer specifying the minimum cell count required for a cluster 
...
# 5.8.1: Total_cnvRegion.DelAmp() calculate the cnv happends in Deletion and 
# Amplification in whole chromosome
#'==============================================================================
...
# 5.9: cnvRegion.toPQarm() input cnvRegions in defined format then add p, q arm 
