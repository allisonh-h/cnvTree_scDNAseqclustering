...
output_bp_covers <- function(Template, binsize, overlap, overlap_times)
{
  ...
  bp2 <- dplyr::bind_rows(bp_list)  # LH: added 12292025
  #print(!(all(dim(bp2) == c(0, 0))))  # LH: added 12292025; FALSE if bp2 = 0x0 
  
  if (!(all(dim(bp2) == c(0, 0))) == TRUE){ # LH: added 12292025
...
# 5.8.1: Total_cnvRegion.DelAmp() calculate the cnv happends in Deletion and Amplification in whole chromosome
...
# 5.9: cnvRegion.toPQarm() input cnvRegions in defined format then add p, q arm information automatically
