##### V. scDNA: scDNA_Output()
##### scDNAclustering_06_OthersAndOutput.R


# 6.1: scDNA.superimpose() function for superimpose between definedCNVs and 
# DNA segment info
#'==============================================================================
...
# 6.2: scDNA.clustering() function for receiving DNA clustering output in 
# superimpose range
#'==============================================================================
...
# 6.3: Totalcluster_pdf() function for creating pdf in total clusters by CN matrix
#'==============================================================================
...
#'  PDF will be saved.
#' @param sexchromosome_plot A logical value. If `TRUE`, the output plot includes 
...
Totalcluster_pdf <- function(Input, Template, pqArm_file, cellcutoff, step = "Subclone", 
                             FILEname, FILEpath, sexchromosome_plot=FALSE)
{
  # select computed column name
  fucStep <- " 6.3_cnvTree_v030"
  DebugMsg(fucStep, "start")
  ...
  for (k in Cluster_No){
    ...
    count = count + 1
    Fig_seq[[count]] <- GenomeHeatmap(Input = Input, cellID = SS, 
...
# 6.3.1: Totalcluster_Cluster_No() function for transfer the number of clusters in the data
#'==============================================================================
...
#' @param cellnum A numeric value defining the minimum number of cells required 
#'  for a cluster to be included.
...
# 6.3.2: Totalcluster_SS() function for transfer each cluster of cells in the data
#'==============================================================================
...
# 6.3.3: GenomeHeatmap() function for plotting CN pattern in each cells
#'==============================================================================
...
GenomeHeatmap <- function(Input, cellID, pqArm_file, sexchromosome = FALSE)
{
  ...
  if (msg == TRUE) {
    print("LH: start looping...6.3.3.1_cnvTree_v030") # LH: added on 02232026
  }
...
# 6.3.3.1: segment_transform() transforms each cell CN-segment information
#'==============================================================================
...
# 6.4: scDNA_CNVpattern() cluster w/ or w/o CNV pattern plot
#'==============================================================================
...
#' (CNV) patterns across clusters, with hierarchical clustering represented by a dendrogram.
#'
...
#' @param smoothing A logical value. If `TRUE`, the heatmap applies smoothing over 
...
scDNA_CNVpattern <- function(Input, final_cluster, cellcutoff, pqArm_file, 
                             FILEpath, FILEname, sexchromosome = FALSE, 
                             smoothing = TRUE)
{
  fucStep <- " 6.4_cnvTree_v030"
  DebugMsg(fucStep, "start")
...
# 6.4.1: generate_dynamic_colormap() color template for heatmaps 
# (Customize color + RColorBrewer template)
#'==============================================================================
