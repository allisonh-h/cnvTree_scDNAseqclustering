...
#' 6.2 endTimed: Calculate time elapsed since start time
#'
...
GenomeHeatmap <- function(Input, cellID, pqArm_file, sexchromosome=FALSE){
  ...
  if (msg == TRUE) {
    print("LH: start looping...6.3.3.1_cnvTree-main") # LH: added on 02232026
  }
...
# 6.3.3.1: Transform each cell CN-segment information
...
# 6.4: writeOutput() function for outputting cluster results in .txt
...
# 6.5: scDNA.superimpose() function for superimpose between definedCNVs and DNA segment info
...
# 6.6: scDNA.clustering() function for receiving DNA clustering output in superimpose range
...
# 6.7: cluster w/ or w/o CNV pattern plot
...
# 6.7.1: color template for heatmaps (Customize color + RColorBrewer template)
